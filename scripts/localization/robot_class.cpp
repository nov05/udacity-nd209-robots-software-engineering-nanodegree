...
private:
    double gen_gauss_random(double mean = 0, double variance = 1)
    {
        // Gaussian random
    ...
    {
        // Probability Density Function (PDF) of x for 1-dim Gaussian with mean mu and var. sigma
...
int main()
{
    ...
    my_robot.set(10.0, 10.0, 0);

    // Printing out the new robot position and orientation
    cout << my_robot.show_pose() << endl;

    // TODO: Rotate the robot counter-clockwise by PI/2.0 and then move him forward by 10.0
...
/*
The so-called "particles" in this context are virtual robot instances. 
Each particle represents a hypothetical robot with its own pose (position and orientation) 
and its own noise characteristics. In particle filtering or Monte Carlo Localization (MCL), 
these particles are used to approximate the true position of a robot in a noisy environment.
*/